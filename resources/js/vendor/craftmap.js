    ...
    $.fn.craftmap = function(options) {
		var D = {
			...
			event_marker: {
				...
				popup: true,
				popup_name: 'popup',
				onClick: function(event_marker, popup){},
				onClose: function(event_marker, popup){}
			},
			controls: {
				init: true,
				name: 'controls',
				onClick: function(event_marker){}
			},
			preloader: {
				init: true,
				name: 'preloader',
				...
				P = {
					...
					_globals: {
						init: function(){
							...
							MARKER = C.find('.'+S.event_marker.name),
					...
					event_marker: {
						...
						set: function(){
							MARKER.each(function(){
								...
								t.css(css);
							}).wrapInner($('<div />').addClass(S.event_marker.name+'Content').hide());
						},
						open: function(){
							MARKER.on('click', function(){
								var t = $(this), id = t.attr('id'), event_marker = S.event_marker, w = t.width(), h = t.height(),
									position = t.position(), x = position.left, y = position.top, id = t.attr('id'),
									html = t.find('.'+event_marker.name+'Content').html();
									
								if (event_marker.center){
									var cy = -y+D.h/2-h/2,
										cx = -x+D.w/2-w/2,
								...
								if (event_marker.popup){
									$('.'+event_marker.popup_name).remove();
									var css = {
										position:'absolute', 
										zIndex:'3'
									}
									t.after(
										$('<div />').addClass(event_marker.popup_name+' '+id).css(css).html(html).append(
											$('<a />').addClass('close')
										)
									);
									var POPUP = t.next('.'+event_marker.popup_name), 
										pw = POPUP.innerWidth(), 
								...
								event_marker.onClick.call(this, t, POPUP); 
								return false;
							});
						},
						close: function(){
							$(document).on('click', '.close', function(){
								var t = $(this), popup = t.parents('.'+S.event_marker.popup_name), event_marker = popup.prev('.'+S.event_marker.name);
								popup.remove();
								P.controls.active.remove();
								S.event_marker.onClose.call(this, event_marker, popup);
								return false;
							});
						}
					},
					controls: {
						...
						set: function(){
							CONTROLS.click(function(){
								...
								div = C.find('.'+ S.event_marker.name).filter('#'+rel);
								div.trigger('click');
